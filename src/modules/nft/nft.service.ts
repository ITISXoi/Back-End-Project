...
import {
    ...
    getRandomFromSet,
    hashString,
    isJsonString, makeCombination, makeCombinationFromMultipleArray,
    smartContract
} from "../../shared/Utils";
...
export class NftService {
    ...
    async createOffchain(data: any, files: any, type: string, creator: User) {
        ...
        const queryBuilder = getConnection()
            ...
            .select('*')
            .where(`nft.image_ids = '${imageIds.toString()}' AND nft.status IS NOT NULL`)
        ...
        nftOffchain.attributes = JSON.stringify(await this.convertAtributes(data.imageIds.toString()));
    ...
    async updateOffchain(data: UpdateOffchain, files: any, nftOffchain: NftOffchain, creator: User) {
        if (!isJsonString(data.imageIds.toString())) {
        ...
        if (data.imageIds && nftOffchain.imageIds != data.imageIds.toString()) {
            ...
            const nftOffchainData = await getConnection()
                ...
                .where('nft_offchain.imageIds = :imageIds', { imageIds: data.imageIds.toString() })
                .andWhere('nft_offchain.creatorId = :creatorId', { creatorId: creator.id })
        ...
        nftOffchain.description = data.description ? data.description : nftOffchain.description;
    ...
    async create(data: Create, files: any, creator: User) {
        ...
        if (Date.now() < collection.startMintTime || Date.now() > collection.endMintTime) {
            throw Causes.COLLECTION_START_END_TIME_INVALID;
        }
        ...
        for (let i = 0; i < images.length; i++) {
            ...
            layerIds.push(images[i].layerId);
            await this.layerRepository.update({ id: images[i].layerId }, { isMinted: true })
        }
        ...
        nft.attributes = JSON.stringify(await this.convertAtributes(data.imageIds.toString()));
        ...
        let contractPrice = Math.round((Number(nft.price) * Math.pow(10, 18))).toString();
        //update tokenUri
        await this.nftRepository.update({ id: nft.id }, nft);
        console.log('nft.tokenUri',nft.tokenUri);
        let signature = await this.getSignature(currencyConfig.chainId, nft.collectionId, data.wallet, contractPrice, data.nextTokenId, fileName, hashUniqueNft);
    ...
    async generate(collectionId: number) {
        ...
        for (let i = 0; i < layerImageCombinations.length; i++) {
            allImageCombinations.push(...makeCombinationFromMultipleArray(layerImageCombinations[i]));
        }

        const validImageCombinations = [];
        for (let i = 0; i < allImageCombinations.length; i++) {
            const hashUniqueNft = '0x' + hashString(allImageCombinations[i].toString() + collection.collectionAddress + collection.chainId);

            const nftData = await getConnection().createQueryBuilder(Nft, 'nft')
                .where('nft.status is not null and nft.layer_hash = :layerHash', { layerHash: hashUniqueNft })
                .getRawOne();

            console.log(allImageCombinations[i].toString(), collection.collectionAddress, collection.chainId);
            console.log(hashUniqueNft);

            const checkImages = await getConnection().createQueryBuilder(Image, 'image')
        ...
        const generatedImages = validImageCombinations[Math.floor(Math.random() * validImageCombinations.length)];
        if (generatedImages) {
            return await getConnection().createQueryBuilder(Image, 'image')
                .leftJoin(Layer, 'layer', 'image.layer_id = layer.id')
                .select(`
                        image.id as id, image.image_url as imageUrl, image.name as name,
                        image.description as description, image.image_type as imageType,
    ...
    async getCombination(totalCombination: number, collectionId: number) {
        ...
        const images = await this.imageRepository
            ...
            .leftJoin(Layer, 'layer', 'image.layer_id = layer.id')
            .where('layer.collection_id = :collectionId', { collectionId: collectionId })
            .select('image.id as imageId, image.remaining_quantity as max, image.layer_id as layerId')
            .execute();
        ...
        return validProduct.slice(0, totalCombination < validProduct.length ? totalCombination : validProduct.length);
    }
    ...
    async generateNtf(collectionId: number) {
        ...
        let combinations = await this.getCombination(collectionData ? collectionData.totalNfts : 10, collectionId);
        let generatedNfts = [];
    ...
    async createJsonFile(data: any, nft: Nft, dataIpfs: any) {
        ...
        await promises_1.writeFile(join(process.cwd(), 'ipfs', fileName), JSON.stringify(jsonData));
        ...
        try {
            if (data) {
                ...
                for (let item of dataConvert) {
                    let image = await getConnection().getRepository(Image).findOne({ id: item });
                    if (!image) continue;
                    let layer = await getConnection().getRepository(Layer).findOne({ id: image.layerId });
    ...
    async getDetailNftOffchain(id: number) {
        let nftOffchain = await getConnection()
            .createQueryBuilder(NftOffchain, 'nft_offchain')
            .leftJoin(Collection, 'collection', `
    ...
    async getListNftOffchains(params, paginationOptions: IPaginationOptions) {
        ...
        if (params.collectionId) {
            queryBuilder.andWhere(`nft_offchain.collection_id = :collectionId`, { collectionId: params.collectionId });
            queryCount.andWhere(`nft_offchain.collection_id = :collectionId`, { collectionId: params.collectionId });
        }

        if (params.collectionKeyId) {
            queryBuilder.andWhere(`nft_offchain.collection_key_id = :collectionKeyId`, { collectionKeyId: params.collectionKeyId });
            queryCount.andWhere(`nft_offchain.collection_key_id = :collectionKeyId`, { collectionKeyId: params.collectionKeyId });
        }

        if (params.collectionAddress) {
            queryBuilder.andWhere(`nft_offchain.collection_address = :collectionAddress`, { collectionAddress: params.collectionAddress });
            queryCount.andWhere(`nft_offchain.collection_address = :collectionAddress`, { collectionAddress: params.collectionAddress });
        }

        if (params.chainId) {
            queryBuilder.andWhere(`nft_offchain.chain_id = :chainId`, { chainId: params.chainId });
            queryCount.andWhere(`nft_offchain.chain_id = :chainId`, { chainId: params.chainId });
        }

        if (params.type) {
            queryBuilder.andWhere(`nft_offchain.type like '%!${params.type.trim()}%' ESCAPE '!'`);
            queryCount.andWhere(`nft_offchain.type like '%!${params.type.trim()}%' ESCAPE '!'`);
        }

        if (params.userId) {
            queryBuilder.andWhere(`nft_offchain.creator_id = :userId`, { userId: params.userId });
            queryCount.andWhere(`nft_offchain.creator_id = :userId`, { userId: params.userId });
        }
    ...
    async n_getListAllNfts(params, paginationOptions: IPaginationOptions) {
        ...
        let queryBuilder = getConnection()
            .createQueryBuilder(Nft, "nft")
            .select(`nft.id, nft.collection_id as collectionId, nft.name as name, nft.token_id,
        ...
        if (params.name) {
            queryBuilder.andWhere(`nft.name like '%!${params.name.trim()}%' ESCAPE '!'`);
            queryCount.andWhere(`nft.name like '%!${params.name.trim()}%' ESCAPE '!'`);
        }

        if (params.chainId) {
            queryBuilder.andWhere(`nft.chainId like '%!${params.chainId.trim()}%' ESCAPE '!'`);
            queryCount.andWhere(`nft.chainId like '%!${params.chainId.trim()}%' ESCAPE '!'`);
        }

        if (params.collectionId) {
            queryBuilder.andWhere(`nft.collectionId like '%!${params.collectionId.trim()}%' ESCAPE '!'`);
            queryCount.andWhere(`nft.collectionId like '%!${params.collectionId.trim()}%' ESCAPE '!'`);
        }

        if (params.contractAddress) {
            queryBuilder.andWhere(`nft.contractAddress like '%!${params.contractAddress.trim()}%' ESCAPE '!'`);
            queryCount.andWhere(`nft.contractAddress like '%!${params.contractAddress.trim()}%' ESCAPE '!'`);
        }

        if (params.userId) {
            queryBuilder.andWhere(`nft.artist_id = :userId`, { userId: params.userId });
    ...
    async getListNfts(params, paginationOptions: IPaginationOptions) {
        ...
        if (params.userId) {
            queryBuilder.andWhere(`nft.creator_id = :userId`, { userId: params.userId });
            queryCount.andWhere(`nft.creator_id = :userId`, { userId: params.userId });
        }
    ...
    handleStatusNft(nft) {
        if (nft && (nft.is_onsale || nft.isOnsale)) return { ...nft, status: "onsale" }
    ...
    async getSignature(chainId, collectionID, sender, fee, tokenId, tokenUri, layerHash) {
        let dataHash = this.encodeData(chainId, collectionID, sender, fee, tokenId, tokenUri, layerHash);
        let signature = null;
        console.log('chainId, collectionID, sender, fee, tokenId, layerHash',chainId, collectionID, sender, fee, tokenId, tokenUri, layerHash);
        let messageHashBinary = ethers.utils.arrayify(dataHash);
        let wallet = new ethers.Wallet('647a9e855888b072d447fab6a0a71fd9534998476e3fd42b62f707eea7df1bcb');
    ...
    encodeData(_chainId, _collectionID, _sender, _fee, _tokenId,_tokenUri, _layerHash) {
        if (!_chainId || !_collectionID || !_sender || !_fee || !_tokenId  || !_layerHash) {
            throw Causes.MISSING_PARAMS_WHEN_GENERATING_SIGNATURE;
        }
        console.log("_chainId,_collectionID, _sender, _fee, _tokenId, _tokenUri, _layerHash", _chainId, _collectionID, _sender, _fee, _tokenId,_tokenUri, _layerHash);
        const payload = ethers.utils.defaultAbiCoder.encode(['uint256', 'uint256', 'address', 'uint256', 'uint256','string', 'bytes'], [_chainId, _collectionID, _sender, _fee, _tokenId,_tokenUri, _layerHash]);
