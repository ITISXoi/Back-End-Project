...
export class CollectionService {
  ...
  async createLayerImages(data: any, files: any, admin: Admin) {
    ...
    try {
      const layerUpdate = await this.layerRepo.findOne({
      ...
      if (layerUpdate) {
        await Promise.all([
          await queryRunner.manager
            ...
            .where("layer.id = :layerUpdateId", {
              layerUpdateId: layerUpdate.id,
            })
            .delete()
            .execute(),
          await queryRunner.manager
            ...
            .where(
              "image.collection_id = :collectionId and image.layer_id = :layerId",
              { collectionId: data.collectionId, layerId: layerUpdate.id }
            )
            .delete()
            .execute(),
        ]);
      ...
      layer.description = insertLayer.description
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        image.probability = imageData[i].probability
          ? imageData[i].probability
          : 0;
  ...
  async updateLayerImages(data: any, files: any, admin: Admin) {
    ...
    try {
      ...
      layerUpdate.description = insertLayer.description
        ? insertLayer.description
        : null;
      ...
      let newImageUrl = [];
      if (layerUpdate) {
        const deleteImageUrlData = await queryRunner.manager
          .createQueryBuilder(Image, "img")
          .select("image_url")
          .where(
            "collection_id = :collectionId and layer_id = :layerId and is_minted = 0",
            { collectionId: data.collectionId, layerId: layerUpdate.id }
          )
          .execute();
      ...
      if (files && files.length > 0) {
        for (let i = 0; i < files.length; i++) {
          ...
          image.quantity = imageData[i].quantity;
          image.remainingQuantity = imageData[i].quantity;
          image.percent = imageData[i].probability
            ? imageData[i].probability
            : 0;
          image.probability = imageData[i].probability
            ? imageData[i].probability
      ...
      if (dataUpdate && dataUpdate.length > 0) {
        for (let i = 0; i < dataUpdate.length; i++) {
          ...
          image.percent = dataUpdate[i].probability
            ? dataUpdate[i].probability
            : 0;
          image.probability = dataUpdate[i].probability
            ? dataUpdate[i].probability
            : 0;
  ...
  async createImage(
    ...
    image.description = insertImage.description
  ...
  async getDetailCollection(id: number) {
    let queryBuilder = getConnection()
      ...
      .leftJoin(
        Nft,
        "nft",
        `(nft.collection_id = collection.collection_id and nft.status = 'confirmed')`
      )
      //   .leftJoin(
      //     CurrencyToken,
      //     "currency_token",
      //     `(currency_token.contract_address = collection.payment_token and currency_token.chain_id =  collection.chain_id)`
      //   )
      .select(
  ...
  async getListImage(
    ...
    if (data.name) {
      queryBuilder.andWhere(`image.name like '%${data.name.trim()}%'`);
      queryCount.andWhere(`image.name like '%${data.name.trim()}%'`);
    }

    if (data.creatorId && data.userType != 1) {
      queryBuilder.andWhere(`image.creator_id = :creatorId`, {
        creatorId: data.creatorId,
      });
      queryCount.andWhere(`image.creator_id = :creatorId`, {
        creatorId: data.creatorId,
      });
    }

    if (data.layerId) {
      queryBuilder.andWhere(`image.layer_id = :layerId`, {
        layerId: data.layerId,
      });
      queryCount.andWhere(`image.layer_id = :layerId`, {
        layerId: data.layerId,
      });
    }
  ...
  async getListLayer(
    ...
    if (data.name) {
      queryBuilder.andWhere(`layer.name like '%${data.name.trim()}%'`);
      queryCount.andWhere(`layer.name like '%${data.name.trim()}%'`);
    }

    if (data.creatorId && data.userType != 1) {
      queryBuilder.andWhere(`layer.creator_id = :creatorId`, {
        creatorId: data.creatorId,
      });
      queryCount.andWhere(`layer.creator_id = :creatorId`, {
        creatorId: data.creatorId,
      });
    }

    if (data.collectionId) {
      queryBuilder.andWhere(`layer.collection_id = :collectionId`, {
        collectionId: data.collectionId,
      });
      queryCount.andWhere(`layer.collection_id = :collectionId`, {
        collectionId: data.collectionId,
      });
    }
    ...
    if (layer && layer.length > 0) {
      for (let i = 0; i < layer.length; i++) {
        let queryBuilderImage = getConnection()
          ...
          .orderBy("image.updated_at", "DESC")
          .where(
            "(image.layer_id = :layerId and image.remaining_quantity > 0)",
            { layerId: layer[i].id }
          );
        const image = await queryBuilderImage.execute();
        layer[i].images = image;
  ...
  async getLayerImagePreview(params: any) {
    let queryBuilder = getConnection()
      ...
      .addSelect("currency_token.currency as currency")
      .where(
        `(layer.collection_id = :collectionId and layer.layer_index = :layerIndex)`,
        { collectionId: params.collectionId, layerIndex: params.layerIndex }
      )
      .orderBy("layer.layer_index", "ASC");
  ...
  async getListCollection(
    ...
    let queryCount = getConnection()
      ...
      .orderBy("collection.updated_at", "DESC");
    if (data.name) {
      queryBuilder.andWhere(`collection.name like '%${data.name.trim()}%'`);
      queryCount.andWhere(`collection.name like '%${data.name.trim()}%'`);
    }

    if (data.creatorId && data.creatorType == 2) {
      queryBuilder.andWhere(`collection.creator_id = :creatorId`, {
        creatorId: data.creatorId,
      });
      queryCount.andWhere(`collection.creator_id = :creatorId`, {
        creatorId: data.creatorId,
      });
    }

    if (!data.isAdmin) {
      queryBuilder.andWhere(`collection.status is not null`);
      queryCount.andWhere(`collection.status is not null`);
    }

    if (!data.creatorId) {
      queryBuilder.andWhere(`collection.is_public = 1`);
      queryCount.andWhere(`collection.is_public = 1`);
    }
  ...
  async updateCollection(
    ...
    if (
      ...
      checkCollectionName.name != collection.name
    ) {
      throw Causes.COLLECTION_NAME_EXISTS;
    }
    if (insertCollection.name) collection.name = insertCollection.name;
    ...
    if (insertCollection.symbol) collection.symbol = insertCollection.symbol;
  ...
  async upload(file, thumbnail_size?): Promise<any> {
    ...
    if (!thumbnail_size)
      ...
      return await this.uploadS3_with_thumbnail(
        file.buffer,
        bucketS3,
        originalname,
        thumbnail_size
  ...
  async deleteS3(bucket: string, keyS3: string, thumbnail?: number) {
    const s3 = this.getS3();
    const key = keyS3;
