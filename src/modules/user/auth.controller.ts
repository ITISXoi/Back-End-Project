...
export class AuthController {
  ...
  async addWallet(
    ...
    const userUpdate = await this.authService.updateProfile(user, {
      wallet: data.wallet,
    });
  ...
  async resendMailActive(
    @Body() data: SendMailResetPassword
  ): Promise<any | EmptyObject> {
  ...
  async get2fa(@Req() request: RequestWithUser): Promise<EmptyObject> {
    ...
    return {
      twoFactorAuthenticationSecret: user.isActive2fa
        ? null
        : this.jwtService.decode(secret),
    };
  }
